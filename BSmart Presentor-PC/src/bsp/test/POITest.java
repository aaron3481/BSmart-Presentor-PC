...
public class POITest{
	public static void main(String[] args) throws Exception {
		...
		//for(int i=0;i<55;i++){
		PackagePart part = slides[0].getPackagePart().getPackage().getPart(PackagingURIHelper.createPartName("/ppt/slides/slide1.xml"));
		
		System.out.println(slides[0].getPackagePart());
		System.out.println(slides[0].getPackagePart().getInputStream());
		
		BufferedReader in = new BufferedReader(new InputStreamReader(slides[0].getPackagePart().getInputStream()));
		String line;
		String result="";
		...
		/*XSLFSlide s = slides[0];
		XSLFShape sp[] = s.getShapes();
		XSLFShape spp = sp[0];
